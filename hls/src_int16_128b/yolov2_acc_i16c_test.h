...
void yolov2_hls_ps(network *net, float *input)
{
	...
	//kernel_w_i16_offset_add kernel_w_i16_Q kernel_w_i16
	uint32_t data_num = get_file_size("kernel_w_rc_i16.bin");
	int16_t *weight_buf = (int16_t *)malloc(data_num);
	read_binfile_flt32_rb((float *)weight_buf, "kernel_w_rc_i16.bin", data_num/2);
	...
	data_num = get_file_size("bias_i16c.bin");
	int16_t *beta_buf = (int16_t *)malloc(data_num);
	printf("%d\n", read_binfile_flt32_rb((float *)beta_buf, "bias_i16c.bin", data_num/2));
	...
	for(int ic=0; ic<LANE_NUM; ic++)
	{
		...
		if((c4*LANE_NUM+ic) >= img_c)
			...
			ifm_buf[idx_out] = tmp_ptr_ft32[idx_in]*tmp_exp;
	}
		...
		if((l.type == CONVOLUTIONAL) || (l.type == MAXPOOL))
		{
			...
			switch(l.type)
			{
				case CONVOLUTIONAL:
					...
					FPGA_Acc((DT_IO*)ifm_ptr, (DT_IO*)ofm_ptr, (DT_IO*)(weight_buf+woffset), (DT_IO *)(beta_buf+boffset),
				...
				case MAXPOOL:
					...
					FPGA_Acc((DT_IO*)ifm_ptr, (DT_IO*)ofm_ptr, NULL, NULL,

...
// void memcpy_ifm(float *ibuf_local,  float *input, uint32_t burstlen, uint32_t offset_base, uint8_t ifm_lmode, uint16_t Input_w, uint32_t IHxIW, uint16_t px_t,
...
// void memcpy_ifm2buf(float input_buffer[Tn][OnChipIB_Height][OnChipIB_Width], float ibuf_local[TnxIB_HxIB_W],
...
// void memcpy_w(float *wbuf_local,  float *weight_addr, uint32_t burstlen){
...
// void memcpy_w2buf(float weight_buffer[Tm][Tn][K][K], float *wbuf_local, uint8_t Ksize, uint8_t TM_MIN, uint8_t TN_MIN){
...
void ofm_mmcpy_cont(float *Output, float local_buf[OFM_BLmax], uint32_t ofm_offset, uint16_t data_num)
{
	float *OFM_base = Output + ofm_offset;
	// memcpy(OFM_base, local_buf, data_num*sizeof(float));
...
void pp_cont(float local_buf[OFM_BLmax], float output_buffer[Tm][Tr][Tc], float *bias_buffer,
				uint8_t TR_MIN, uint8_t TC_MIN, uint32_t OHxOW, uint16_t ofm_w, bool IsNL, bool IsBias,
				uint16_t m, uint32_t offset, int burstlen, uint32_t blmax_tc, uint16_t T2_MIN,
